...
# Import du modèle
try:
    from models.mobilenet import MobileNetV3Classifier
except ImportError:
    st.error("Erreur: Impossible d'importer MobileNetV3Classifier. Vérifiez que le fichier models/mobilenet.py existe.")

# Chemin du modèle
MODEL_PATH = "checkpoints/best_mobilenet.pt"
...
MODEL_METRICS = {
    ...
    "fps": 56,
    "model_size": 2.54  # MobileNetV3-Small est plus petit que Large
}
...
def load_model():
    ...
    try:
        model = MobileNetV3Classifier(num_classes=2, pretrained=False, version='small')  # CHANGÉ: large → small
...
def show_home():
    ...
    **MobileNetV3** - Architecture légère optimisée pour mobile avec:
    ...
    **PKLot Dataset** - Ensemble complet d'images de stationnement avec:
    ...
    with st.expander("🌐 Déployer cette app publiquement"):
        ...
        **4. Obtenir l'URL publique**
        - Exemple: `https://your-app.streamlit.app`
